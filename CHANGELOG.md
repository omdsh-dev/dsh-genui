...
- **GenUI 回复语言保持会话语言**：常驻提示强化会话语言规则并使用 `<user-language ...>` 示例值，skill catalog metadata 使用语言中立说明，`validate_dsh_ui` 结果与围栏修复消息使用带 `reply_language=preserve` 的结构化诊断；`SKILL.md` 中高诱导性的用户可见示例值改用必须替换的语言占位符（#165）。
