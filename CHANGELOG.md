...
### 修复
- **DOM 围栏诊断与 DSH 2.0.9 兼容（issue #158）**：`dsh-ui` 标签比较统一忽略前后空白，流式接管到落定复核使用同一规则；兼容 `data-code-block-banner` / `data-code-block-content` 包装的单 `<pre>` 表面。已知表面被消息级段落或多个代码体安全守卫拒绝时输出一次结构化告警（仅含 `preCount` 与围栏外块级标签，不含围栏正文）；对接近 `dsh-ui` 但未精确匹配的标签提供受约束的一次性诊断，普通代码块不会刷屏。保留 issue #13/#19 的 `pre.closest(...)` 安全边界。
